...
class swift():
	...
	def led_detector(self, image):
		...
		self.centroid=list(map(lambda x,y:((x-0.5)*24*0.15)+y,self.centroid,self.drone_position))
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		if max(list(map(abs,swift_drone.error[0:2])))<0.4 and swift_drone.error[2] < 1 and swift_drone.setpoint in points: #or swift_drone.setpoint == [10.2, 10.4, 37.5]:
			if itr<len(points):
				if itr!=0:
					swift_drone.led_detector(swift_drone.current_frame)
					if var % 2 == 0:
						swift_drone.past_led = swift_drone.num_led
						var+=1
				if swift_drone.num_led > 0:
		...
		if var % 2 != 0:
			swift_drone.num_led = max([swift_drone.num_led, swift_drone.past_led])

		if max(list(map(abs,swift_drone.error[0:2])))<0.2 and swift_drone.error[2] < 1 and swift_drone.setpoint not in points and swift_drone.num_led > 0 and swift_drone.num_led != swift_drone.num_det_led:
			if swift_drone.past_centroid != []:
				for i in (swift_drone.past_centroid):
					if abs(swift_drone.centroid[0] - i[0]) < 2 and abs(swift_drone.centroid[1] - i[1]) < 2:
						swift_drone.past_cen_check = False

			if swift_drone.past_cen_check == True and var % 2 != 0:
				location.organism_type=swift_drone.identify()
				swift_drone.past_centroid.append(swift_drone.setpoint)
				location.whycon_x=swift_drone.setpoint[0]
				location.whycon_y=swift_drone.setpoint[1]
				location.whycon_z=swift_drone.setpoint[2]
				pub.publish(location)
				swift_drone.num_det_led = swift_drone.num_led
