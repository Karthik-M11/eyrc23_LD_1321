...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		if max(list(map(abs,swift_drone.error)))<max_error or swift_drone.setpoint == [10.2, 10.4, 37.5]:
			if(itr<len(points) and swift_drone.num_led==0):
				if itr!=0:
				...
				if(step==0):
					...
					max_error = 0.2
					step=1
					print(f'To centroid :{swift_drone.setpoint}')
